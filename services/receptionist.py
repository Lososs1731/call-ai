...
{chr(10).join([f"{i+1}. {msg}" for i, msg in enumerate(bot_messages)])}
...
{chr(10).join([f"   • \"{phrase}\"" for phrase in data['successful_phrases'][-8:]])}

🎯 BEST PRACTICES:
{chr(10).join([f"   • {bp}" for bp in data['best_practices'][-5:]])}
